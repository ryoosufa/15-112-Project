...
#    Name      : Rija
#    AndrewID  : ryoosufa
#    File Created: 20/10/2016
#    Modification History:
#    Start             End
#    6/11 4 pm        6/11 6 pm
#    9/11 12 am       9/11 4 am
#   10/11 2:30 pm        10/11 3 pm
#    11/11 5 pm        11/11 9 pm
#    12/11 8 am        12/11 9 am
#    13/11 4 pm       13/11 6 pm
#    14/11 12 pm       14/11 1:30 pm
#    15/11 3 pm        15/11 7 pm
#    16/11 5 pm        16/11 9 pm
#    17/11 8 am        17/11 9:30 am
#    18/11 4 pm        18/11 8 pm
#    19/11 12 pm       19/11 1:30 pm
#    20/11 3 pm        20/11 11 pm
#    21/11 5 pm        21/11 9 pm
#    22/11 8 am        22/11 10 am
#    23/11 4 pm        23/11 7 pm
#    24/11 3 pm       24/11 10 pm
#    25/11 2 pm        25/11 7 pm
#    26/11 5 pm        26/11 9 pm
#    27/11 9 am        27/11 10 pm
...
def getMail(s):
    ...
    # write the original contents
        ...
        if isfile("search.txt"):
            ...
            DownloadFile=open(F+"protocol",'w')
...
# when the open button on the window with recipes from all users is pressed this function is called
# this just reads from the already present recipes and display it
# in the required format
# many functions below are defined which are identical to this one except the window from the which they are being called
# because that window contains the listbox with the name of any recipe the user
def openAll():
...
# this is incase the user doesn't want to use up extra space and decides to delete a recipe from their computer
# which was added by other users who are members of the cookbook club
# this function will work in the local directory
# so deleting a recipe from the maincookbook will not delete it from all the people using the app
def delfromAll():
    global recBox

    #recBox is the listbox which has all the recipes
    # All is initialized as an empty list

    All=[]
    #delall will have the one clicked to be deleted from .get(ACTIVE)
    # and .delete will just instantly remove the name from the listbox to show you that it is there no more

    delall=recBox.get(ACTIVE)
    recBox.delete(ACTIVE)
    All.append(delall)
    # then these recipe names must be deleted from file which are read each time the app is run
    # these are the "Allnames.txt" and "names.txt" file
    # also "search.txt" must not have it incase you search for some recipe by the username after it has been deleted

    # if the file exixts it opens it as oldfile and reads from it, simultaneously opening another file called "newfile.txt"
    # in the "All" list is the one to be deleted

    
    if exists("Allnames.txt"):
        with open('Allnames.txt') as oldfile, open('newfile.txt', 'w') as newfile:
            # for each element in All, which will  be one element usually, it writes to new file, whatever line was in the file opened as oldfile
            # provided it doesn't contain anything from the list "All" in it

            for line in oldfile:
                if not any(bad_word in line for bad_word in All):
                    newfile.write(line)

        # the oldfile is closed as it was then it is removed using "os.remove"
        oldfile.close()
        os.remove("Allnames.txt")
    # the new file is renamed and this is how it has been modified after deletion
    os.rename("newfile.txt","Allnames.txt")
    # the same process is then repeated to delete the name of the recipe from the "search.txt" fil
    ...
    # the same process is then repeated to delete the name of the recipe from the "names.txt" file
    if exists("names.txt"):
    ...
    # finally the for loop looks for every file in the recipes folder
    # if the recipe that was clicked to be deleted(delall) is in one of the filenames that file must be removed
...
# this is for when the "main cookbook" window is destroyd , the recBox(the listbox in it)
...
def All_recipes():
    ...
    #disabling buttons if the there are no recipes
...
def del_fav():
    ...
    # this if follows the same procedure as the one described above for the "delfromAll" as it has to remove from the file
    # read to display favourites
    # but deleting from favourites doesn't delete the recipe itself
...
def FavsWnd():
    ...
    if isfile("Favlist.txt"):
     ...
     # disabling buttons incase there are no recipes in the listbox
    if len(list(favBox.get(0,END)))==0:
        butn.config(state="disabled")
        butnn.config(state="disabled")
    # setting the listbox with recipes in favourites to None as soon as  it is destroyed when this window is closed        
    favourites.bind('<Destroy>',winClosed)
    
    favourites.mainloop()
# this is the function responsible to set favBox (the lisbox with the favourites) to None
def winClosed(event):
    global favBox
    favBox=None

# this is the function responsible to set Box1 (the lisbox with the (grocery list) to None
def windowClosed(event):
    global Box1
    Box1=None


# this function works exactly the same way as the function called "del_fav"
# this function opens the window
# which has a listbox with the names of recipes
# that were added to favourites
# it is called when the button from the main menu called
# "Grocery List" is pressed
def Delgro():
    # Box1 is the listbox which displays
    # the names of recipes added by the user
    # to Grocery List and .delete(ACTIVE) only removes this
    # name from this listbox
    # ACTIVE means the only one  that has been selected by the user
    global Box1
    listdel=[]
    ...
    listdel.append(deld)
    # this if follows the same procedure as the one described above for the "delfromAll" as it has to remove from the file
    # read to display Grocery list items
    # but deleting from the Grocery List doesn't delete the recipe itself
...
def openGrocery():
    ...
    if listItems!=None and (len(listItems))!=0:
        ...
        for i in range (len(listItems)):
            # if makes sure names in the listbox are not repeated
    ...
    Box1.pack()
    # this button "Btn2" calls the Delgro function 
    Btn2=Button(show,text="Already Bought!", command=Delgro,background="black",fg="white")
    Btn2.pack()

    # if the file "Grocerylist.txt" exists the following is executed
    ...
    # disabling buttons in case there is no recipe displayed
    if (len(Box1.get(0,END)))==0:
        Btn2.config(state="disabled")
    # this window when closed must make the grocery listbox None for that it calls "windowClosed"
...
# this is responsible to set the listbox called "Box" to None when the window with "My Recipes" is destroyed
def wnClosed(event):
    global Box
    Box=None
...
def delR():
    ...
    Box.delete(ACTIVE)
    r.append(rem)
    #Box is the listbox which has all the recipes
    # r is initialized as an empty list
    
    #rem will have the one clicked to be deleted from .get(ACTIVE)
    # and .delete will just instantly remove the name from the listbox to show you that it is there no more
    # add the recipe to be deleted to the list called "r"
    ...
    if isfile("names.txt"):
        # for each element in r, which will  be one element usually, it writes to new file, whatever line was in the file opened as oldfile
    # provided it doesn't contain anything from the list "All" in it
        ...
        os.rename("newfile.txt","names.txt")
            # the oldfile is closed as it was then it is removed using "os.remove"
        # the new file is renamed and this is how it has been modified after deletion
# the same process is then repeated to delete the name of the recipe from the "Allnames.txt" file
    ...
    # finally the for loop looks for every file in the recipes folder
    # if the recipe that was clicked to be deleted(r) is in one of the filenames that file must be removed
    for filename in os.listdir(os.getcwd()+"\\recipes"):
...
def recipebox():
    ...
    favsBtn.pack()
    delbtn.pack()
    # disabling buttons in case there are no recipes
    if len(Box.get(0,END))==0:
        openBtn.config(state="disabled")
        favsBtn.config(state="disabled")
        delbtn.config(state="disabled")
        # window when destroyed calls the function wnClosed so the listbox with recipes is set to None
...
def openingIt():
    ...
    # disabling buttons incase there are no search results and adding that line to the listbox to tell the user
...
# when the open button on the search results window after you searched for a recipe using the ingredient name, window is pressed, this function is called
# it just reads from the already present recipes and displays them in the required format
def open_r():
    cwd=os.getcwd()
    global listb
...
# when the "find" button is clicked on the window where the user searches for the recipe using
# the user,this is called
# it takes what the user enters as the search keywords
def searchByuser():
    global TB
    global list_B
    ...
    # entered sttored what the  user enters into the textbox called "TB"
    ...
    empty=[]
    # if the file exists it needs to search for the user in each line of that file and give results
    if exists("search.txt"):
        z=open("search.txt")
        ls=z.readlines()
        z.close()
        # each line has : user$filename
        # hence you look for the $ symbol
        for f in range (len(ls)):
            ...
            F_N=F_N.strip()
            N_FN=(N,F_N)
            # append a tuple to the list initialized, the tuple has the (username, filename)
            empty.append(N_FN)

        # in the list "empty" now you look for the user which was searched for
        # each tuple that has the user at its [0] index will have a recipe added by that user at index[1]
        for u in range(len(empty)):
            if entered in empty[u][0].lower():
                # make sure the results don't show repeated names
    ...
    # in case no results, display the "no matches.." message and disable the open recipe button
...
# this function opens the search bar window in which the user can type in the name of the user
# for whose recipes they are looking for and press the "Find" button beside the search bar
# the search bar is an entry box and the find button calls the above defined function called "searchByuser"
# to show results after looking for the entered username
def search_user():
...
# this  function is responsible to add recipe names to favourites
#  list i.e. the listbox which contains all the ingredients
# which the user added to from different recipes
# initializing an empty list and a global variable favBox (as they are used in this function):
listFavz=[]
favBox=None
def go_favz():
    global favBox
    ...
    # the .get(ACTIVE) returns the element in a listbox which has been selected
    item=Box.get(ACTIVE)
    # if the file "Favlist.txt" is already present 
    if isfile("Favlist.txt"):
        # reads from it everything it originally contained
            # then it adds the new name with the format i created : each line will have an ingredient name, with two !! before the name begins
            # the names are added to separate lines using f.write("!!"+item+"\n") 
            # after it writes this , it makes sure the data originally in the file is not lost, then it writes that back to the file
    ...
    # in case the user adds something to the favourites list and this listbox
    # called favBox has not been created then it equals None
    # because favBox is created by a function which is called when
    # the "Favourites" button from the main menu is pressed and
    # this is pressed only when the user wants to view the items
    # in the favourites

    # if it is not pressed then whatever recipe the user
    # adds to the Favourites will be stored in a list and
    # that list will be checked for elements(recipe names)
    # when favBox will be created ie whenever the user views the favourites
    if favBox==None:
        ...
        if len(present)==0:
            ...
            for i in range (len(present)):
                if item not in present:
                        # in case it adds the same recipe name twice this if condition is used
                    favBox.insert(END,item)
...
def windowClosed(event):
    # Box1 is the listbox which contains the grocery list items, this function is bound to the window
# that contains this listbox and when that is closed it must change the listbox "Box1" to None
    global Box1
    Box1=None
...
# which the user added to from different recipes
# initializing an empty list and a global variable Box1 (as they are used in this function):
...
def go_shopping():
    ...
    # if the file "Grocerylist.txt" is already present 
    item=ingBox.get(ACTIVE)
    if isfile("Grocerylist.txt"):
        # reads from it everything it originally contained
            # then it adds the new name with the format i created : each line will have an ingredient name, with two !! before the name begins
            # the names are added to separate lines using f.write("!!"+item+"\n") 
            # after it writes this , it makes sure the data originally in the file is not lost, then it writes that back to 
        f = open("Grocerylist.txt")
        text = f.read()
        f.close()
...
# when the open button on the window with the listbox displaying the favourites is pressed this function is called
# this just reads from the already present recipes  in the recipes folder and displays the recipe that was clicked
# in the required format
# the favourites window contains the listbox with the name of any recipe that was added to favourites
# and since the user wants to open and display, the open button is clicked 
def oFavs():
...
def open_rcp():
    ...
    listt=[]
    cn=""
    if isfile("recipes\\"+protocol_file.replace(" ","")):
...
# when the "find" button is clicked on the window where the user searches for the recipe using
# the recipe name,this is called
# it takes what the user enters as the search keywords
...
def openList():
    ...
    # disabling buttons to open the recipe if no recipe exists and adding
    # a string as a message for the user to tell them no matches were found
...
# when the open button on the window with the recipe from the "Feeling Hungry" window is pressed this function is called
# this just reads from the already present recipe in the listbox and displays it, if the user clicks the "click to satisfy your cravings" button,
# in the required format
...
# this is called when the user wants to generate the name of another random recipe in case they didn't want to try the one they
# already got when they clicked "Feeling Hungry?" for once
...
def Randrec():
    ...
    if exists("Allnames.txt"):
        ...
        l_rd=l_rd.strip()
        # this time, since the Listbox called "LB" which shows the randomly generated recipe's name, is
        # not empty, it deletes the one which was there before and then inserts the name of the new recipe
...
# this is called when the button the mainmenu "Feeling Hungry?" is pressed
# it gives a random recipe from all the recipes in the cook book club
def hungry():
    global LB
    # initializing empty list "t"
    t=[]
    dialogB=Tk()
    dialogB.configure(background="bisque")
    LB=Listbox(dialogB,width="30",height="10")
    # if the "Allnames.txt" file exists .readlines() is used to store a list of strings which are the lines
    if exists("Allnames.txt"):
        f=open("Allnames.txt")
        Lineslist=f.readlines()
        f.close()
        ...
        if len(Lineslist)>0:
            # for loop iterates through each element of the list which is the line in the file
            # but since the names were saved with "//" in the beginning of each name

            for i in range(len(Lineslist)):
                # v is a variable that is each element of the list in the loop
                # .replace() function gets rid of these "//" and .strip() removes new line characters
                v=Lineslist[i]
                v=v.strip()
                v=v.replace("//","")
                # if used incase two names are added twice to the list "t"
                if v not in t:
                    t.append(v)

            # a function from the random library is being called, this "choice" function returns a random element from a list
            # in this case it returns the name of a recipe which is stored in a variable called "l_rd"

            l_rd=random.choice(t)
            l_rd=l_rd.strip()
            # this recipe name is displayed in the listbox called LB which was in the window opened when the "Feeling Hungry" button was pressed
            if l_rd not in LB.get(0,END):
                LB.insert(0,l_rd)
    LB.pack()
    ...
    # this button calls a function called "openhungry" which opens the recipe that was randomly generated and displayed
    OBtn=Button(dialogB,text="Click and Satisfy Your Cravings!!!",command=openhungry,background="black",fg="white")
    OBtn.pack()
    # this calls another function called "Randrec" which does the above procedure again and generates another recipe name the same way
    again=Button(dialogB,text="Don't feel like it? Try your luck again!",command=Randrec,background="black",fg="white")
    again.pack()
    # disabling buttons in case there was no recipe
    if len(list(LB.get(0,END)))==0:
...
def start():
    ...
    # before the .after function starts calling  the check function again, the menu must be bound to the function WC, which sets Txt and menu
    # to None , since they don't exist anymore, note that Txt is the textbox which is used for the Newsfeed
    # this is done so that in case the user closes the menu window before the first 7 seconds have passed since the app was run
    # the textbox and window both change to None because they are being used in functions (that access the window and the textbox when some
    # thing is to be inserted/displayed in the textbox)
...
# reponsible to set the textbox for the newsfeed on the "menu" window to none and also the window to None since it won't exist anymore
...
def check():
    ...
    if Txt!=None:
        ...
        # in case no one sent a recipe recently 
        if len(anything)==0:
            Txt.insert("1.0","No new recipes as yet!")
     
    if menu!=None:
        #the variable tuples is list of tuples containing users and filenames
        ...
        Files=[]
        # for loop to go over entire list of tuples that was returned from getMail 
        for i in range (len(tuples)):

            # each tuple at [0]  is the username and at [1] is the name of the file which is (recipe name followed by the word "protocol")
            user = tuples[i][0]
            filee = tuples[i][1]
            filee=filee.replace("protocol","")
            # for adding to the newsfeed , if the user who added the recipe is the user who is logged in then the
            # newsfeed will show "you" in the text box instead of your username

            if user==u:
                user="you"
            displaymsg=user+" just added a recipe:"+filee+"\n"
            
            # incase no one sent a recipe recently
            ...
            if "recipes" in displaymsg:
                ...
                displaymsg=user+" just added a recipe for:"+displaymsg[Ind:]
                 ...
                 # adding to the textbox which is the Newsfeed to show who sent which recipe 
            Txt.insert(1.0,displaymsg)
        
        # .after function is bound to the window called menu which is the mainmenu
        # this is like a  timer to check for recipes ever 7 seconds
        # this function calls itself every 7 seconds 

        menu.after(7000,check)
    else:
        # if menu doesn't exist the .after function is bound to the window called root which is the startup window
        # this is like a  timer to check for recipes ever 7 seconds
        # this function calls itself every 7 seconds 
...
# this is called when the "About" button on the startup window is clicked
def Help():
    
    wn=Toplevel()
    wn.title("How to Use")
    #adding background picture which has all the instructions for how to use the app
    pat = 'about.jpg'
