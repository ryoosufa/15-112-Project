...
def getMail(s):
    ...
    # write the original contents
            ...
            print F, "printed after removing the path"
...
class recipe:
    ...
    def addRecipe(self):
        ...
        print len(name_re), "nameeeeee", list(name_re)
...
##        openBtn.config(state="disabled")
    ...
    global ingBox
    name=lb.get(ACTIVE)
    print list(name.strip()), len(list(name.strip))
    #if len(list(name.strip()))!=0:
    protocol_file=join(cwd+"\\recipes",name+"protocol")
    '''File=open(protocol_file)
    text=File.read()
